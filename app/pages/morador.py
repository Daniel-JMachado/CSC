...
class MoradorPage:
    ...
    def render_inicio(self):
        ...
        # FILTRO CIRÚRGICO: Contar apenas coletas deste morador_id específico
        total_coletas = len(coletas_df)
        
        # Pendentes: status='pendente' (ainda não foi aceita por nenhum catador)
        coletas_pendentes = len(coletas_df[coletas_df['status'] == 'pendente']) if 'status' in coletas_df.columns else 0
        
        # Concluídas: status='concluida' (catador já fez a coleta)
    ...
    def render_solicitar_coleta(self):
        ...
        # Ordenar alfabeticamente por nome (mas mantendo "Disponível para Todos" no topo)
        catadores_especiais = [c for c in catadores if c.get('especial', False)]
        catadores_normais = sorted([c for c in catadores if not c.get('especial', False)], key=lambda x: x['nome'])
        catadores = catadores_especiais + catadores_normais
                        ...
                        with st.form(key=f"form_coleta_{catador.get('id', idx)}", clear_on_submit=True):
                            ...
                            if submitted:
                                ...
                                if not endereco_coleta:
                                    ...
                                    # Verificar se é solicitação para todos ou para catador específico
                                    catador_id_selecionado = catador.get('id') if not catador.get('especial', False) else None
                                    
                                    # Dados da solicitação
                                    coleta_data = {
                                        'morador_id': self.user_data.get('id'),
                                        'catador_id': catador_id_selecionado,
                                    ...
                                    if success:
                                        ...
                                        # Criar notificações de acordo com o tipo de solicitação
                                        if catador_id_selecionado is None:
                                            # Solicitação para TODOS os catadores do bairro
                                            st.info("📢 Sua solicitação foi disponibilizada para todos os catadores do seu bairro.")
                                            st.info("🔔 Assim que um catador aceitar, você receberá uma notificação.")
                                            
                                            # Enviar notificação para todos os catadores que atendem o bairro do morador
                                            users = load_users()
                                            catadores_bairro = users[users['tipo'] == 'catador']
                                            bairro_morador = self.user_data.get('bairro', '')
                                            
                                            for _, cat in catadores_bairro.iterrows():
                                                # Verificar se o catador atende o bairro
                                                areas_cat = str(cat.get('areas_atuacao', '')).split(',')
                                                areas_cat = [a.strip().lower() for a in areas_cat if a.strip()]
                                                
                                                if bairro_morador.lower() in areas_cat or not areas_cat:
                                                    nova_notificacao = {
                                                        "usuario_id": cat['id'],
                                                        "tipo_usuario": "catador",
                                                        "titulo": "Nova solicitação de coleta disponível",
                                                        "mensagem": f"Nova solicitação no bairro {bairro_morador} para o dia {data_coleta.strftime('%d/%m/%Y')} ({horario_coleta}). Materiais: {', '.join(tipo_material)}. Quantidade: {quantidade_estimada:.1f} kg",
                                                        "data": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                                        "lida": False
                                                    }
                                                    save_notificacao(nova_notificacao)
                                        else:
                                            # Solicitação para catador ESPECÍFICO
                                            st.info(f"📬 Sua solicitação foi enviada para {catador.get('nome')}.")
                                            st.info("🔔 Você será notificado quando o catador responder.")
                                            
                                            # Criar notificação para o catador específico
                                            nova_notificacao = {
                                                "usuario_id": catador_id_selecionado,
                                                "tipo_usuario": "catador",
                                                "titulo": "Nova solicitação de coleta",
                                                "mensagem": f"{self.user_data['nome']} solicitou uma coleta para o dia {data_coleta.strftime('%d/%m/%Y')} ({horario_coleta}). Materiais: {', '.join(tipo_material)}. Quantidade estimada: {quantidade_estimada:.1f} kg",
                                                "data": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                                "lida": False
                                            }
                                            save_notificacao(nova_notificacao)
    ...
    def render_minhas_coletas(self):
        ...
        try:
            ...
            coletas_aceitas = coletas_df[coletas_df['status'].isin(['agendada', 'concluida'])]
            
            # Criar tabs para organizar as coletas
            tab1, tab2 = st.tabs([f"⏳ Pendentes ({len(coletas_pendentes)})", f"✅ Aceitas ({len(coletas_aceitas)})"])
            ...
            with tab2:
                ...
                if not coletas_aceitas.empty:
                    st.info("💬 Estas coletas foram aceitas pelos catadores. Use o chat para se comunicar!")
                    self.mostrar_lista_coletas(coletas_aceitas, users_df, "aceita")
                else:
    ...
    def mostrar_lista_coletas(self, coletas, users, tipo):
        ...
        for idx in range(len(coletas)):
            ...
            # Obter dados do catador (solicitado ou que aceitou)
            catador_nome = "🌍 Disponível para Todos os Catadores"  # Default para coletas sem catador
            
            # Se tem catador_id definido (coleta específica ou aceita)
            if pd.notna(coleta.get('catador_id')):
                catador_row = users[users['id'] == coleta['catador_id']]
                if not catador_row.empty:
                    catador_nome = catador_row['nome'].iloc[0]
                    # Adicionar status se ainda está pendente
                    if tipo == "pendente":
                        catador_nome = f"👤 {catador_nome} (aguardando resposta)"
                    else:
                        catador_nome = f"✅ {catador_nome}"
                else:
                    catador_nome = "❓ Catador não encontrado"
            elif tipo == "pendente":
                # Coleta sem catador específico ainda pendente
                catador_nome = "🌍 Disponível para Todos (aguardando)"
            ...
            with st.container():
                ...
                elif coleta.get('status') == 'agendada':
                    status_emoji = "📅"
                    status_color = "#3498db"
                ...
                with col2:
                    st.markdown(f'<span style="background: {status_color}; color: white; padding: 4px 8px; border-radius: 12px; font-size: 0.8rem; font-weight: bold;">{status_text}</span>', unsafe_allow_html=True)
                ...
                # Para coletas aceitas (agendadas ou concluídas), mostrar chat (se tem catador definido)
                if pd.notna(coleta.get('catador_id')) and tipo == "aceita":
