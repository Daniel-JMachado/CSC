...
class CatadorPage:
    ...
    def render_inicio(self):
        ...
        # FILTRO CIRÚRGICO: Contar apenas coletas onde este catador é o responsável
        # Total: onde catador_id == id do catador logado E status = concluida
        coletas_concluidas = len([c for c in coletas if c['status'] == 'concluida' and c.get('catador_id') == catador_id])
        
        # Pendentes: coletas disponíveis para este catador (status='pendente' E (catador_id=None OU catador_id=catador_id))
        # Essas são as que aparecem na aba "Solicitações Disponíveis"
        bairros = self.user_data.get('areas_atuacao', '').split(',')
        bairros = [b.strip() for b in bairros if b.strip()]
        coletas_disponiveis_df = get_coletas_disponiveis(bairros, catador_id=catador_id)
        coletas_pendentes = len(coletas_disponiveis_df) if not coletas_disponiveis_df.empty else 0
        
        total_coletas = coletas_concluidas  # Total é apenas as concluídas por este catador
        
        # Calcular peso total de resíduos coletados (apenas deste catador)
        peso_total = sum([float(c.get('peso_kg', 0)) for c in coletas if c['status'] == 'concluida' and c.get('catador_id') == catador_id])
    ...
    def mostrar_solicitacoes_recentes(self):
        ...
        # Passa o ID do catador para buscar coletas disponíveis para ele
            ...
            for coleta in coletas_recentes:
                ...
                with col1:
                    if st.button(f"Aceitar solicitação #{coleta['id']}", key=f"accept_{coleta['id']}"):
                        ...
                        # Criar notificação para o morador (sem especificar ID - será auto-incrementado)
                        nova_notificacao = {
                        ...
                        morador_row = users[users['id'] == coleta['morador_id']]
                        morador_nome_simples = morador_row['nome'].iloc[0] if not morador_row.empty else "Morador"
    ...
    def render_solicitacoes_disponiveis(self):
        ...
        coletas_disponiveis_df = get_coletas_disponiveis(bairros_filtrados, catador_id=self.user_data.get('id'))
    ...
    def mostrar_lista_coletas(self, coletas, users, tipo):
        ...
        for coleta in coletas:
            # Obter dados do morador que solicitou a coleta
            morador_row = users[users['id'] == coleta['morador_id']]
                    ...
                    if st.button(f"Aceitar coleta #{coleta['id']}", key=f"accept_{coleta['id']}", type="primary"):
                        ...
                        for _, admin in admins.iterrows():
                            notificacao_admin = {
                                "usuario_id": admin['id'],
                                "tipo_usuario": "admin",
                                "titulo": "Nova coleta aceita no sistema",
                                "mensagem": f"O catador {self.user_data['nome']} aceitou a coleta #{coleta['id']} do morador {morador_nome.replace('👤 ', '')}. Bairro: {morador_bairro}. Materiais: {coleta.get('tipos_materiais', coleta.get('materiais', 'Não especificado'))}. Quantidade: {coleta.get('quantidade_estimada', coleta.get('peso_kg', 'N/A'))} kg.",
                                "data": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                "lida": False
                            }
                            save_notificacao(notificacao_admin)
