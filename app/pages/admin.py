...
class AdminPage:
    ...
    def show_dashboard(self):
        ...
        with col2:
            ...
            # FILTRO CIRÚRGICO: Separar corretamente as coletas por status do sistema
            if not coletas.empty and 'status' in coletas.columns:
                # Agendadas: Coletas que catador ACEITOU mas ainda não concluiu
                agendadas = len(coletas[coletas['status'] == 'agendada'])
                
                # Pendentes: Coletas que NENHUM catador aceitou ainda (aguardando)
                em_andamento = len(coletas[coletas['status'] == 'pendente'])
                
                # Concluídas: Coletas finalizadas pelos catadores
            ...
            <div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); 
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    ...
                    <div style="background: rgba(255,255,255,0.1); padding: 15px; border-radius: 10px; backdrop-filter: blur(10px);">
                        ...
                        <div style="opacity: 0.9; font-size: 0.9rem;">Pendentes</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.1); padding: 15px; border-radius: 10px; backdrop-filter: blur(10px);">
        ...
        # Seção de Notificações de Coletas Aceitas
        st.markdown("---")
        st.subheader("🔔 Atividade Recente do Sistema")
        
        from app.utils.database import get_notificacoes_by_usuario
        
        # Buscar notificações do admin
        notificacoes = get_notificacoes_by_usuario(self.user_data['id'], 'admin')
        
        if not notificacoes.empty:
            # Ordenar por data (mais recentes primeiro)
            notificacoes = notificacoes.sort_values(by='data', ascending=False)
            
            # Separar em abas: Não Lidas e Todas
            tab1, tab2 = st.tabs([f"🔴 Não Lidas ({len(notificacoes[notificacoes['lida'] == False])})", 
                                   f"📋 Todas ({len(notificacoes)})"])
            
            with tab1:
                notif_nao_lidas = notificacoes[notificacoes['lida'] == False]
                if not notif_nao_lidas.empty:
                    for _, notif in notif_nao_lidas.head(10).iterrows():
                        with st.expander(f"🔴 {notif.get('titulo', 'Notificação')} - {notif.get('data', '')}"):
                            st.markdown(f"**Mensagem:** {notif.get('mensagem', '')}")
                            if st.button(f"Marcar como lida", key=f"mark_read_{notif.get('id')}"):
                                from app.utils.database import marcar_notificacao_como_lida
                                marcar_notificacao_como_lida(notif.get('id'))
                                st.success("Notificação marcada como lida!")
                                st.rerun()
                else:
                    st.info("✅ Nenhuma notificação nova no momento.")
            
            with tab2:
                for _, notif in notificacoes.head(20).iterrows():
                    icon = "✅" if notif.get('lida') else "🔴"
                    with st.expander(f"{icon} {notif.get('titulo', 'Notificação')} - {notif.get('data', '')}"):
                        st.markdown(f"**Mensagem:** {notif.get('mensagem', '')}")
                        if not notif.get('lida', False):
                            if st.button(f"Marcar como lida", key=f"mark_read_all_{notif.get('id')}"):
                                from app.utils.database import marcar_notificacao_como_lida
                                marcar_notificacao_como_lida(notif.get('id'))
                                st.success("Notificação marcada como lida!")
                                st.rerun()
        else:
            st.info("Nenhuma notificação no sistema ainda.")
    ...
    def show_statistics(self):
        ...
        if not coletas.empty:
            ...
            with col4:
                # Peso total coletado - APENAS DE COLETAS CONCLUÍDAS
                peso_total = 0.0
                if not coletas.empty and 'status' in coletas.columns:
                    # Filtrar apenas coletas concluídas
                    coletas_concluidas_df = coletas[coletas['status'] == 'concluida']
                    
                    if not coletas_concluidas_df.empty and 'peso_kg' in coletas_concluidas_df.columns:
                        # Somar peso_kg das coletas concluídas
