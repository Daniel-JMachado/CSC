...
def get_coletas_disponiveis(bairros=None, catador_id=None):
    """
    Retorna as coletas disponíveis para um catador.
    Inclui:
    1. Coletas "Disponível para Todos" (catador_id = None) - TODOS os catadores veem, sem filtro de bairro
    2. Coletas específicas atribuídas ao catador (se catador_id for fornecido)
    
    Args:
        bairros (list, optional): Lista de bairros para filtrar as coletas. Default é None.
        catador_id (int, optional): ID do catador para buscar coletas específicas. Default é None.
    ...
    # Filtrar apenas coletas pendentes (não concluídas ou recusadas)
    ...
    coletas_sem_catador = coletas_pendentes[coletas_pendentes['catador_id'].isna() | coletas_pendentes['catador_id'].isnull()]
    ...
    if catador_id is not None:
        ...
        # Combinar coletas sem catador (todos veem) + coletas específicas (filtradas por bairro)
        coletas_disponiveis = pd.concat([coletas_sem_catador, coletas_especificas], ignore_index=True)
        
        # Remover duplicatas (caso existam)
        coletas_disponiveis = coletas_disponiveis.drop_duplicates(subset=['id'])
    else:
        # Se não foi fornecido catador_id, retornar apenas coletas sem catador
        coletas_disponiveis = coletas_sem_catador
    
    return coletas_disponiveis
